...
pub trait FutureExt: TryFuture + Unpin + Sized {
...
impl<F: TryFuture + Unpin> FutureExt for F {}
    ...
    F::Error: From<io::Error>,
{
    ...
    fn poll(
        ...
        match TryFuture::try_poll(Pin::new(&mut self.future), cx) {
            Poll::Pending => {}
            other => return other,
        }
...
/// timing out execution and such.
pub trait StreamExt: TryStream + Unpin + Sized {
...
impl<S: TryStream + Unpin> StreamExt for S {}
    ...
    S::Error: From<io::Error>,
{
    ...
    fn try_poll_next(
        ...
        let dur = self.dur;
        let mut pin = Pin::new(&mut self.stream);
        let r = pin.as_mut().try_poll_next(cx);
        match r {
            Poll::Pending => {}
            other => {
        ...
        let mut pin = Pin::new(&mut self.timeout);
        if pin.as_mut().poll(cx)?.is_ready() {
            pin.as_mut().reset(dur);
